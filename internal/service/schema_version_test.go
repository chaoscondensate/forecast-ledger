...
func TestLoadRejectsMissingUnknownAndFutureSchemaVersionsFirst(t *testing.T) {
	tests := []struct {
		...
		{name: "superseded v2", content: `{"schema_version":"2.0.0"}`},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !ok || applicationErr.Details["supported_schema_version"] != "2.0.1" {
...
func TestMutatingOperationRejectsUnsupportedVersionBeforeWriting(t *testing.T) {
	...
	unsupported := bytes.Replace(raw, []byte(`"schema_version": "2.0.1"`), []byte(`"schema_version": "2.0.0"`), 1)
...
func TestSupersededV2LifecycleEvidenceRejectsBeforeEffects(t *testing.T) {
  ...
  "questions": [{
	...
	if transport.requests != 0 {
		t.Fatalf("unsupported v2.0.0 contacted the network %d times", transport.requests)
	}
	for _, forbidden := range []string{storage.LedgerLockPath(path), keyPath, filepath.Join(directory, "proofs"), output} {
		if _, err := os.Stat(forbidden); !os.IsNotExist(err) {
			t.Fatalf("unsupported v2.0.0 created %s: %v", filepath.Base(forbidden), err)
		}
	}
	after, err := os.ReadFile(path)
	if err != nil || !bytes.Equal(after, old) {
		t.Fatalf("unsupported v2.0.0 ledger changed: %v", err)
	}
}
